...
# def embedding_concat(x, y, patchsize=3):
...
#     unfolded_features = unfolded_features.reshape(*z.shape[:1], patchsize, patchsize, -1)
#     unfolded_features = unfolded_features.permute(0, 3, 1, 2)
...
def embedding_concat(x, y, patchsize=3):
    ...
    unfolded_features_x = unfolded_features_x.reshape(*x.shape[:1], patchsize, patchsize, -1)
    x = unfolded_features_x.permute(0, 3, 1, 2)
    unfolded_features_y = unfolder(y)
    unfolded_features_y = unfolded_features_y.reshape(*y.shape[:1], patchsize, patchsize, -1)
    y = unfolded_features_y.permute(0, 3, 1, 2)
    # ----------
    B, C1, H1, W1 = x.size()
    _, C2, H2, W2 = y.size()
    s = int(H1 / H2)
...
class PatchCore(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx): # save locally aware patch features
        ...
        embedding = embedding_concat(embeddings[0], embeddings[1], args.patchsize)
    ...
    def test_step(self, batch, batch_idx): # Nearest Neighbour Search
        ...
        for feature in features:
            ...
            # embeddings.append(m(feature))
            embeddings.append(feature)
        embedding_ = embedding_concat(embeddings[0], embeddings[1], args.patchsize)
        ...
        # ----------
        # anomaly map
        anomaly_map = score_patches[:, 0].reshape((args.patchsize, args.patchsize))

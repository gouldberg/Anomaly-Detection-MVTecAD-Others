...
class Ocr_Gan(BaseModel):
    ...
    def forward_d(self):
        ...
        """
        self.pred_real, self.feat_real = self.netd(self.input_res + self.input_lap)
    ...
    def backward_g(self):
        ...
        self.err_g_con = self.opt.w_con * self.l_con(self.fake, self.input_res+self.input_lap)
    ...
    def test(self, plot_hist=False):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.data.valid, 0):
                ...
                rec = (self.input_res + self.input_lap - self.fake).view(si[0], si[1] * si[2] * si[3])
